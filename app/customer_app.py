...
# ---------------- HOME ----------------
    ...
    st.session_state.favorites = get_user_favorites(
    ...
    most_common = [
        name for name, _ in Counter(ordered_names).most_common()
    ]

    recommended = [
        f for f in foods
        if f["name"] not in most_common
    ][:4]
    ...
    if recommended:
        ...
        for i, food in enumerate(recommended):
            with cols[i % 4]:
                ...
                st.markdown(
                    f"<div class='capsule'>{food.get('description','')}</div>",
                    unsafe_allow_html=True
                )
    ...
    filtered = (
        [f for f in foods if search.lower() in f["name"].lower()]
        if search
        else foods
    )
...
# ---------------- DETAILS PAGE ----------------
    ...
    st.markdown(
        "<div class='header'>Food Details</div>",
        unsafe_allow_html=True
    )
...
# ---------------- ORDER FORM ----------------
    ...
    st.markdown(
        "<div class='header'>Enter Delivery Details</div>",
        unsafe_allow_html=True
    )

    st.markdown(
        "<div class='black-label'>Full Name</div>",
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        "<div class='black-label'>Mobile Number</div>",
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        "<div class='black-label'>Address</div>",
        unsafe_allow_html=True
    )
    ...
    if st.button("Submit Order"):
        ...
        try:
            ...
            if order_id:
                ...
                st.session_state.order_id = str(order_id)
...
# ---------------- REVIEW PAGE ----------------
    ...
    st.markdown(
        "<div class='black-header'>Rate & Review</div>",
        unsafe_allow_html=True
    )

    st.markdown(
        "<div class='review-label'>Select Rating</div>",
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        "<div class='review-label'>Write your feedback</div>",
        unsafe_allow_html=True
    )
    ...
    if st.button("Submit Review"):
        ...
        try:
            ...
            if review_result and review_result.inserted_id:
                ...
                st.error(
                    "❌ Review could not be saved to the database."
                )

        except Exception as e:

            st.error(
                f"❌ Review could not be saved: {e}"
            )
